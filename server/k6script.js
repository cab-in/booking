...
export default function () {
  ...
  for (const i of Array(RPSperVU).keys()) {
    http.get(`http://localhost:3001/api/bookings/${listings[Math.floor(Math.random() * (99999 - 0) + 0)]}`);
    // http.post(`http://localhost:3001/api/bookings/${listings[Math.floor(Math.random() * (99999 - 0) + 0)]}`, payload, params);
}
